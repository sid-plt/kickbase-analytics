"""Confirmed, team-scoped Kickbase display-name mappings for KBStats players."""
...
def _validate(frame: pd.DataFrame) -> pd.DataFrame:
    if list(frame.columns) != list(REFERENCE_COLUMNS):
        raise ValueError(f"Kickbase cross references must use columns {list(REFERENCE_COLUMNS)}")
    ...
    if invalid.any():
        raise ValueError("Kickbase cross references contain empty required fields.")
    mapping_key = list(zip(output["canonical_team"].str.casefold(), output["kbstats_name"].map(_key)))
    if pd.Series(mapping_key).duplicated().any():
        raise ValueError("Kickbase cross references contain duplicate team/KBStats mappings.")
    return output


def load_references(path: Path = REFERENCE_PATH) -> pd.DataFrame:
    """Load confirmed manual mappings without involving ID-based provider references."""
    if not path.exists():
    ...
    try:
        ...
        raise ValueError(f"Could not read Kickbase cross references {path}: {exc}") from exc
    return _validate(frame)


def persist_references(references: pd.DataFrame, path: Path = REFERENCE_PATH) -> None:
    """Validate and atomically replace the separate Kickbase mapping store."""
